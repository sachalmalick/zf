...
def logistic_regression(X, y, name="Learnign Curve for Logistic Regression"):
    ...
    model = LogisticRegression(random_state=0, max_iter=40000, C=hp["C"], penalty='l2').fit(X_train, y_train)
    cross_validation_score = cross_val_score(model, X, y, cv=3)
    y_pred = model.predict(X_test)
    accuracy = accuracy_score(y_test, y_pred)

    cv = ShuffleSplit(n_splits=50, test_size=0.35, random_state=0)
...
def kmeans(X, name="Learnign Curve for KMEANS"):
    scaler = StandardScaler()
    X = scaler.fit_transform(X)
...
def lda(X, y, name="Learnign Curve for LDA"):
    scaler = StandardScaler()
    X = scaler.fit_transform(X)
    X_train, X_test, y_train, y_test = train_test_split(
...
def random_forest(X, y, name="Learnign Curve for Logistic Regression"):
