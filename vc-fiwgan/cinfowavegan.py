...
def WaveGANQ(
  ...
  # [16384, 1] -> [4096, 64]
  output = x
  with tf.compat.v1.variable_scope('Qdownconv_0'):
    output = tf.compat.v1.layers.conv1d(output, dim, kernel_len, 4, padding='SAME')
  output = lrelu(output)
  output = phaseshuffle(output)
  ...
  # Layer 1
  # [4096, 64] -> [1024, 128]
  with tf.compat.v1.variable_scope('Qdownconv_1'):
    output = tf.compat.v1.layers.conv1d(output, dim * 2, kernel_len, 4, padding='SAME')
    output = batchnorm(output)
  output = lrelu(output)
  output = phaseshuffle(output)
  ...
  # Layer 2
  # [1024, 128] -> [256, 256]
  with tf.compat.v1.variable_scope('Qdownconv_2'):
    output = tf.compat.v1.layers.conv1d(output, dim * 4, kernel_len, 4, padding='SAME')
    output = batchnorm(output)
  output = lrelu(output)
  output = phaseshuffle(output)
  ...
  # Layer 3
  # [256, 256] -> [64, 512]
  with tf.compat.v1.variable_scope('Qdownconv_3'):
    output = tf.compat.v1.layers.conv1d(output, dim * 8, kernel_len, 4, padding='SAME')
    output = batchnorm(output)
  output = lrelu(output)
  output = phaseshuffle(output)
  ...
  # Layer 4
  # [64, 512] -> [16, 1024]
  with tf.compat.v1.variable_scope('Qdownconv_4'):
    output = tf.compat.v1.layers.conv1d(output, dim * 16, kernel_len, 4, padding='SAME')
    output = batchnorm(output)
  output = lrelu(output)
  ...
  if slice_len == 32768:
    # Layer 5
    # [32, 1024] -> [16, 2048]
    with tf.compat.v1.variable_scope('Qdownconv_5'):
      output = tf.compat.v1.layers.conv1d(output, dim * 32, kernel_len, 2, padding='SAME')
      output = batchnorm(output)
    output = lrelu(output)
  elif slice_len == 65536:
  ...
  # Connect to single logit
  with tf.compat.v1.variable_scope('Qoutput'):
    Qoutput = tf.compat.v1.layers.dense(output, num_categ)
